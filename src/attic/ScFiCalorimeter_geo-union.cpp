...
std::tuple<Volume, Position> build_module(const Detector &desc, const xml::Component &mod_xml, SensitiveDetector &sens)
{
  ...
  for (int iy = 0; iy < ny; iy++) {
    ...
    for (int ix = 0; ix < nx; ix++) {
      ...
      fiberUnionShape = UnionSolid(fiberUnionShape, fiberShape, Position{x, -y, 0});
      fiberCladUnionShape = UnionSolid(fiberCladUnionShape, fiberCladShape, Position{x, -y, 0});
    }
  }

  Volume fiberVol("fiber_vol", fiberUnionShape, fiberMat);
  Volume fiberCladVol("fiberClad_vol", fiberCladUnionShape, fiberMat);
  fiberVol.setSensitiveDetector(sens);

  auto fiberPV = modVol.placeVolume(fiberVol, Position{-sx/2.+eps, sy/2.-eps, 0});
  modVol.placeVolume(fiberCladVol, Position{-sx/2.+eps, sy/2.-eps, 0});
  fiberPV.addPhysVolID("fiber_x", 1).addPhysVolID("fiber_y", 1);
