...
void place_layer(const std::string &prefix, bool is_even, double height, double yabs, double yshift,
    const Volume &modVol, const Material &modMat, const Volume &fiberCladVol,
...
std::tuple<Volume, Position> build_module(const Detector &desc, const xml::Component &mod_xml, SensitiveDetector &sens)
{
  ...
  place_layer("Top", false, yt, sy/2.-yt/2., -yt/2.+fdisty/2.,
      modVol, modMat, fiberCladVol, fdistx, fr, sx, sz, foffx);
  ...
  for (iy = 0; iy < ny; iy++) {
    ...
    place_layer(Form("Middle%d",iy), is_even, fdisty, sy/2.-y, 0.,
        modVol, modMat, fiberCladVol, fdistx, fr, sx, sz, foffx);
  }
  ...
  if (sy - y < foffy - eps) {
    ...
    bool is_even = iy%2 == 0 ? true : false;
    place_layer("Bottom", is_even, yb, -sy/2.+yb/2., yb/2.-fdisty/2.,
        modVol, modMat, fiberCladVol, fdistx, fr, sx, sz, foffx);
  }
    ...
    double fdistx, double fr, double sx, double sz, double foffx)
{
  ...
  if (is_even) {
    ...
    modVol.placeVolume(layerLeftVol, Position{-sx/2.+xl/2., yabs, 0});
  }
  else {
    ...
    Volume layerLeftVol(prefix + "LayerLeft_vol", layerLeftShape, modMat);
    modVol.placeVolume(layerLeftVol, Position{-sx/2.+xl/2., yabs, 0});
    modVol.placeVolume(fiberCladVol, Position{-sx/2.+foffx, yabs + yshift, 0});
  }
  ...
  for (int ix = 0; ix < nx; ix++) {
    ...
    modVol.placeVolume(fiberVol, Position{-sx/2.+x, yabs, 0});
    modVol.placeVolume(fiberCladVol, Position{-sx/2.+x, yabs + yshift, 0});
  }
  ...
  if (sx - x < foffx - eps) {
    ...
    modVol.placeVolume(layerRightVol, Position{sx/2.-xr/2., yabs, 0});
  }
  else {
